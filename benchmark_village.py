...
#     p_t = GwGt.estimate_target_distribution({0: p_s}, dim_t=num_partitions)
    ...
    time_s = time.time()
    sub_costs, sub_probs, sub_idx2nodes, trans = GwGt.graph_partition_gd(cost,
                                                                      p_s,
                                                                      p_t,
                                                                      database['idx2node'],
                                                                      ot_dict)
...
mutual_info = process_sgwl_village(cost,database,num_nodes,num_partitions);
end = time.time()
...
mis = []
rt = []
ts = [8.053]#np.linspace(7,9,20)
for t in ts:
    start = time.time()
    cost = sgw.undirected_normalized_heat_kernel(nG,t)
    mi = process_sgwl_village(cost,database,num_nodes,num_partitions);
    mis.append(mi)
    end = time.time()
    rt.append(end-start)
...
with open('res_benchmark_village.txt', 'w') as outfile:
