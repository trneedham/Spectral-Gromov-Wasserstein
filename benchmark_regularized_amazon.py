...
def process_sgwl_amazon(cost_s,database,num_nodes,num_partitions,beta=2e-7,verbose=False):
    p_s = np.zeros((num_nodes, 1))
    p_s[:, 0] = np.sum(cost_s, axis=1) ** 0.005
    p_s /= np.sum(p_s)
    ...
    time_s = time.time()
    sub_costs, sub_probs, sub_idx2nodes, trans = GwGt.graph_partition(cost_s,
                                                                      p_s,
                                                                      p_t,
                                                                      idx2node,
                                                                      ot_dict)
...
mutual_info = process_sgwl_amazon(cost,database,num_nodes,num_partitions,beta=4e-3);
end = time.time()
...
for t in ts:
    ...
    mi = process_sgwl_amazon(cost,database,num_nodes,num_partitions,beta=1.5e-6);
    mis.append(mi)
    end = time.time()
    rt.append(end-start)
...
with open('res_benchmark_regularized_amazon.txt', 'w') as outfile:
